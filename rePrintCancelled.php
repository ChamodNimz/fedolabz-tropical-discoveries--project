...
<body>
  ...
  <div class="jumbotron">
    <h1 class="display-4 text-warning shadow-lg">Print Cancelled voucher</h1>
  </div>
    ...
    <table class="table p-5 mt-5 table-striped" id="table-printVoucher" >
      ...
      <tbody>
        <?php 
            ...
            $query="SELECT `ID_H`, `voucherNo`, `voucherDate`, `hotelName`, `status`, `guestName` FROM `voucherh` where status='cancelled' ";
  ...
  $(document).ready(function() {
    ...
    $('#table-printVoucher tbody').on('click','td',function(){
        ...
        id_h=table.cell(this).data().replace('<button class="btn btn-sm btn-info mr-3">Print</button>','');
          ...
          window.open("core/printer.php?id="+id_h+"&crp=1","_blank");
          id_h=0;
    });
